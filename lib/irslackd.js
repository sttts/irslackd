...
class Irslackd {
  ...
  async onIrcClose(ircUser, hadError) {
    ...
    try {
      ...
      // PATCHED: only set away if RTM was started (not for probes)
      if (ircUser.slackWeb && ircUser.rtmStarted) {
  ...
  async onIrcUser(ircUser, msg) {
    ...
    // PATCHED: map own Slack user ID to IRC nick so @mentions are translated
    ircUser.slackToIrc.set(auth.user_id, ircUser.ircNick);

    // PATCHED: Keep configured nick, don't override from Slack
    // (Nick changes before 001 confuse some IRC clients)
    ...
    // PATCHED: Send full IRC welcome sequence immediately after init
    self.ircd.write(ircUser.socket, 'irslackd', '001', [ ircUser.ircNick, ':Welcome to irslackd ' + ircUser.ircNick ]);
    self.ircd.write(ircUser.socket, 'irslackd', '002', [ ircUser.ircNick, ':Your host is irslackd, running version 1.0' ]);
    self.ircd.write(ircUser.socket, 'irslackd', '003', [ ircUser.ircNick, ':This server was created today' ]);
    self.ircd.write(ircUser.socket, 'irslackd', '004', [ ircUser.ircNick, 'irslackd', '1.0', 'o', 'o' ]);
    self.ircd.write(ircUser.socket, 'irslackd', '005', [ ircUser.ircNick, 'LINELEN=' + self.config.lineLen, ':are supported by this server' ]);
    self.ircd.write(ircUser.socket, 'irslackd', '375', [ ircUser.ircNick, ':- irslackd Message of the day -' ]);
    self.ircd.write(ircUser.socket, 'irslackd', '372', [ ircUser.ircNick, ':- Slack-to-IRC bridge' ]);
    self.ircd.write(ircUser.socket, 'irslackd', '376', [ ircUser.ircNick, ':End of MOTD' ]);

    // RTM will be started when #_openclaw is joined (only real connections do this)
  }
  ...
  async onIrcJoinOne(ircUser, ircChan, alreadyInSlackChanId) {
    ...
    // PATCHED: Virtual keepalive channel - no Slack backend
    if (ircChan === '#_openclaw') {
      this.ircd.write(ircUser.socket, ircUser.ircNick, 'JOIN', [ ircChan ]);
      this.ircd.write(ircUser.socket, 'irslackd', '332', [ ircUser.ircNick, ircChan, ':OpenClaw keepalive channel' ]);
      this.ircd.write(ircUser.socket, 'irslackd', '353', [ ircUser.ircNick, '=', ircChan, ':' + ircUser.ircNick ]);
      this.ircd.write(ircUser.socket, 'irslackd', '366', [ ircUser.ircNick, ircChan, ':End of /NAMES list' ]);
      ircUser.mapIrcToSlack(ircChan, '_virtual_openclaw');
      // PATCHED: Start RTM here (only real connections join #_openclaw, not probes)
      ircUser.rtmStarted = true;
    ...
    // PATCHED: block IRC-client-initiated joins to non-allowlisted public channels
    if (this.config.channels && !alreadyInSlackChanId && !this.isMpdmChannel(ircChan) && !this.config.channels.has(ircChan)) {
      console.log('Refusing join to non-allowlisted channel: ' + ircChan);
      return;
    }
  ...
  async initialize(ircUser) {
    ...
    try {
      // PATCHED: skip global user list (too slow for large workspaces)
      // await slackInit.initUsers.call(this, ircUser);
      // PATCHED:       // PATCHED:       await slackInit.initTeams.call(this, ircUser);
      // PATCHED: skip channel list (too slow)
      // await slackInit.initChannelsLite.call(this, ircUser);
  ...
  async onSlackReady(ircUser, event) {
    // PATCHED: 001/005/376 already sent in onIrcUser after initialize()
    // Just set presence here
